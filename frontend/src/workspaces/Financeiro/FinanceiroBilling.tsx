...
const getTrendForRecord = (record: BillingRecord, allRecords: BillingRecord[]) => {
  const branchRecords = allRecords
    .filter(r => r.branch === record.branch)
    .sort((a, b) => a.date.localeCompare(b.date));

  const index = branchRecords.findIndex(r => r.id === record.id);
  if (index <= 0) return 'none';

  const previousRecord = branchRecords[index - 1];
  if (record.value > previousRecord.value) return 'up';
  if (record.value < previousRecord.value) return 'down';
  return 'equal';
};

const FinanceiroBilling: React.FC = () => {
  const { data: billingList = [] } = useBillingRecords();
  ...
  const filteredList = useMemo(() => {
    return billingList.filter(item => {
      ...
      // 2. Branch Select Filter
      const matchesBranch = branchFilter === 'Todas' || item.branch === branchFilter;

      // 3. Date Range Filter
      let matchesDates = true;
      if (startDate) {
        matchesDates = matchesDates && item.date >= startDate;
      }
      if (endDate) {
        matchesDates = matchesDates && item.date <= endDate;
      }

      return matchesText && matchesBranch && matchesDates;
    });
  }, [billingList, searchQuery, branchFilter, startDate, endDate]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', padding: '4px' }}>
      ...
      <header className="view-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <h1 style={{ margin: 0, fontSize: '24px', fontWeight: 700, color: '#0f172a' }}>Faturamento Diário por Filial</h1>
        </div>
      ...
      <div className="reports-filters-bar" style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <div className="reports-filter-right">
          <span className="reports-records-count"><strong>{filteredList.length}</strong> Registros</span>
        </div>
      </div>
            ...
            <tbody>
              {paginatedList.length === 0 ? (
                ...
                paginatedList.map((item, idx) => {
                  ...
                  const parts = item.date.split('-');
                  const formattedDate = parts.length === 3 ? `${parts[2]}/${parts[1]}/${parts[0]}` : item.date;
                  const trend = getTrendForRecord(item, billingList);
