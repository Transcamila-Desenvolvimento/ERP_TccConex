...
const FinanceiroAdjustments: React.FC = () => {
  const { user } = useAuth();
  const { data: adjustments = [] } = useCashAdjustments();
  ...
  // Data State
  const [filtered, setFiltered] = useState<CashAdjustment[]>([]);
  ...
  useEffect(() => {
    ...
    if (filterType !== 'Todos') {
      temp = temp.filter(a => a.type === filterType);
    }
    ...
    setFiltered(temp);
  }, [adjustments, search, filterDate, filterType]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', padding: '4px' }}>
      ...
      <header className="view-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <h1 style={{ margin: 0, fontSize: '24px', fontWeight: 700, color: '#0f172a' }}>Ajustes de Caixa</h1>
        </div>
      ...
      {/* Pagination */}
      <div id="adjustments-pagination" style={{ flexShrink: 0, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '8px', padding: '8px 10px', fontSize: '12px', color: '#475569', borderTop: '1px solid #e2e8f0', background: '#f8fafc', marginTop: '2px' }}>
        <span style={{ fontWeight: 500, marginRight: '4px' }}>
          Página <span id="adj-page-num" style={{ color: '#0f172a', fontWeight: 600 }}>{clampedPage}</span> de <span id="adj-page-total" style={{ color: '#0f172a', fontWeight: 600 }}>{totalPages}</span>
        </span>
        <button 
          type="button" 
