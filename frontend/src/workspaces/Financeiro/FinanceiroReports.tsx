...
import type { PagarRow, ReceberRow, AgingRow } from '../../types/domain';
import { REPORT_TEMPLATE_HINTS, type ReportImportResult } from '../../services/reportImportService';
import {
  useReportBatches,
  usePagarReport,
  useReceberReport,
...
export default function FinanceiroReports() {
  ...
  const { data: batches = [] } = useReportBatches();
  ...
  const { data: pagarPage } = usePagarReport(queryParams, activeTab === 'pagar');
  const { data: receberPage } = useReceberReport(queryParams, activeTab === 'receber');
  const { data: agingPage } = useAgingReport(queryParams, activeTab === 'aging');
  const { data: facets } = useReportFacets(activeTab);

  const activePage = activeTab === 'pagar' ? pagarPage : activeTab === 'receber' ? receberPage : agingPage;
  ...
  const activeBatch = batches.find(b => b.isActive) ?? batches[0] ?? null;
  ...
  const handleStartBatchImport = async () => {
    ...
    if (!activeBatch) {
      alert('Nenhum lote ativo encontrado.');
      return;
    }
    ...
    for (const type of toImport) {
      ...
      try {
        const result = await importReport.mutateAsync({ batchId: activeBatch.id, type, file });
  ...
  const handleFinishBatchImport = async () => {
    if (!activeBatch || !allImportSuccess) return;
    try {
      await finalizeBatch.mutateAsync(activeBatch.id);
  ...
  const renderTableRow = (row: any, idx: number) => {
    ...
    const tdNum = (val?: number) => (
      <td className="num text-end" style={{ borderBottom: '1px solid #f1f5f9', borderRight: 'none' }}>
        {val !== undefined ? val.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : ''}
      </td>
    );
    ...
    if (activeTab === 'pagar') {
      ...
      return (
        <tr key={idx} className={zebraClass}>
          ...
          {tdNum(r.valor)}
          {tdNum(r.saldo)}
          {td(<span style={{ fontSize: '12px', color: '#475569' }}>{r.historico}</span>)}
        </tr>
      );
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', padding: '4px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <div className="reports-dropdown-wrapper">
            ...
            <div className={`reports-dropdown-menu ${isActionsOpen ? 'show' : ''}`}>
              ...
              <span className="reports-dropdown-item" onClick={() => alert('Preparando impressão do relatório...')}>Imprimir Relatório</span>
            </div>
          </div>
        </div>
      </header>
            ...
            <thead>
              {activeTab === 'pagar' && (
                <tr>
                  ...
                  {['Filial','Cód. Forn.','Fornecedor','Título','Tipo','Emissão','Vencto','Vencto Real','Valor','Saldo','Histórico'].map(col => (
                    <th key={col} style={{ borderRight: 'none', borderBottom: '1px solid #e2e8f0', color: '#94a3b8', fontWeight: 500 }}>{col}</th>
                  ))}
                </tr>
              )}
              {activeTab === 'receber' && (
                <tr>
                  ...
                  {['Filial','Cód. Cliente','Cliente','Título','Natureza','Emissão','Vencto','Vencto Real','Valor','Saldo','Histórico'].map(col => (
                    <th key={col} style={{ borderRight: 'none', borderBottom: '1px solid #e2e8f0', color: '#94a3b8', fontWeight: 500 }}>{col}</th>
                  ))}
                </tr>
              )}
              {activeTab === 'aging' && (
                <tr>
                  ...
                  {['Origem','Cód. Cliente','Cliente','Loja','Docto','Série','Tipo','Emissão','Vencto','Região','Total'].map(col => (
                    <th key={col} style={{ borderRight: 'none', borderBottom: '1px solid #e2e8f0', color: '#94a3b8', fontWeight: 500 }}>{col}</th>
                  ))}
                </tr>
              )}
      ...
      <div style={{ flexShrink: 0, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '8px', padding: '8px 10px', fontSize: '13px', color: '#475569', borderTop: '1px solid #e2e8f0', background: '#f8fafc', marginTop: '2px' }}>
        <span style={{ fontWeight: 500, marginRight: '4px' }}>
          ...
          <span style={{ color: '#94a3b8', marginLeft: '8px', fontSize: '12px' }}>({totalItems} registros)</span>
        </span>
        <button
          type="button"
          ...
          <div className="search-modal-card" style={{ width: '700px' }}>
            ...
            <div className="search-input-wrapper" style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                {batchStep === 'import' && activeBatch && (
                  <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                    Lote atual: <strong>{activeBatch.label}</strong> · {activeBatch.date}
                  </p>
                )}
              </div>
              {!isBatchImporting && (
              ...
              <div style={{ padding: '20px 24px 0' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                  ...
                  <tbody>
                    {batches.map((batch, i) => {
                      const isLatest = i === 0;
                      return (
                        <tr key={batch.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 1 ? '#fafafa' : 'white', opacity: isLatest ? 1 : 0.65 }}>
                ...
                <div style={{ marginTop: '8px' }}>
                  <span style={{ fontSize: '11px', color: '#94a3b8' }}>
                    CP = Contas a Pagar · CR = Contas a Receber · AL = Aging Luft · últimos {MAX_BATCHES} lotes
                  </span>
                </div>
