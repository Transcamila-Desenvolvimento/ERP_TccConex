...
def _normalize_filial(value) -> str:
    ...
    if not raw:
        return ''
    padded = raw.zfill(2)
    return FILIAL_CODE_MAP.get(padded) or FILIAL_CODE_MAP.get(raw) or f'Filial {raw}'
...
def _normalize_origem(value) -> str:
    ...
    if not raw:
        return ''
    return ORIGEM_CODE_MAP.get(raw) or ORIGEM_CODE_MAP.get(str(int(raw)) if raw.isdigit() else raw) or f'Origem {raw}'


def _read_rows(file_bytes: bytes, report_type: str):
    ...
    return list(best_sheet.iter_rows(values_only=True))
...
def import_report_file(batch: ReportBatch, report_type: str, file_bytes: bytes, file_name: str) -> dict:
    ...
    rows = _read_rows(file_bytes, report_type)
    if not rows:
    ...
    if report_type == 'pagar':
        ...
        for r, row in enumerate(rows[header_idx + 1:], start=header_idx + 2):
            ...
            saldo = saldo_atual + saldo_vencido or valor
